  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          { label: 'Total procesado', value: aud.total.toLocaleString(), color: 'var(--text-primary)' },
          { label: '✓ Alta confianza', value: `${aud.pctAlta}%`, color: '#166534' },
          { label: '~ Confianza media', value: `${aud.pctMedia}%`, color: '#d97706' },
          { label: '↓ Baja confianza', value: `${aud.pctBaja}%`, color: '#dc2626' },
        ].map(s => (
      ...
      <div className="px-5 py-4 rounded-xl text-sm font-medium" style={{ background: 'rgba(0,212,255,0.05)', color: '#00d4ff', border: '1px solid rgba(0,212,255,0.12)' }}>
        💡 Impacto estimado: <strong>{aud.ahorroEstimado}</strong> — al reducir entregas fallidas y mejorar la calidad de la base de datos.
      </div>

      {/* Search */}
      <div className="card p-5">
        <div className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>Normalizador de Direcciones</div>
        <div className="flex gap-2">
          <div className="relative flex-1">
            <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: 'var(--text-muted)' }} />
            <input type="text" value={inputValue} onChange={e => setInputValue(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && inputValue && handleSearch(inputValue)}
              placeholder="Ingresa una dirección, ej: Broadway 1234, Nueva York"
        ...
        <div className="mt-3">
          <span className="text-[10px] font-semibold uppercase tracking-widest mr-2" style={{ color: 'var(--text-secondary)' }}>Ejemplos:</span>
          {geoExamples.map(g => (
      ...
      {loading && (
        <div className="card p-8 text-center">
          <div className="flex items-center justify-center gap-2 text-sm" style={{ color: 'var(--text-secondary)' }}>
            ...
            Verificando base de datos y geocodificando…
          </div>
        </div>
      )}
      ...
      {result && !loading && (
        <div className="card p-5 space-y-4">
          <div className="flex items-center justify-between">
            <div className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>Resultado de Normalización</div>
          ...
          <div className="flex items-center gap-3 px-4 py-3 rounded-xl"
            ...
            <div>
              ...
              <div className="text-xs mt-0.5" style={{ color: 'var(--text-secondary)' }}>Fuente: {result.fuente}</div>
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between text-xs">
              <span style={{ color: 'var(--text-muted)' }}>Puntuación de confianza</span>
          ...
          {result.normalizada && (
            <div className="grid grid-cols-2 gap-3">
              <div className="p-3 rounded-xl" style={{ background: 'var(--surface-1)' }}>
                <div className="text-[10px] font-semibold uppercase tracking-widest mb-1" style={{ color: 'var(--text-secondary)' }}>Normalizada</div>
                <div className="text-xs" style={{ color: 'var(--text-primary)' }}>{result.normalizada}</div>
              </div>
              <div className="p-3 rounded-xl" style={{ background: 'var(--surface-1)' }}>
                <div className="text-[10px] font-semibold uppercase tracking-widest mb-1" style={{ color: 'var(--text-secondary)' }}>Datos Geográficos</div>
                <div className="text-xs space-y-0.5" style={{ color: 'var(--text-secondary)' }}>
                  <div>Provincia: <span style={{ color: 'var(--text-primary)' }}>{result.provincia}</span></div>
                  <div>Localidad: <span style={{ color: 'var(--text-primary)' }}>{result.localidad}</span></div>
                  <div>Coordenadas: <span style={{ color: 'var(--text-primary)' }} className="font-mono">{result.lat.toFixed(4)}, {result.lng.toFixed(4)}</span></div>
                </div>
              </div>
            </div>
          )}
          ...
          {result.problema && (
            <div className="flex items-start gap-2 p-3 rounded-xl text-xs" style={{ background: '#3d2d0a' }}>
              ...
              <div>
                <div className="font-semibold" style={{ color: '#d97706' }}>Nota:</div>
      ...
      {/* Quality bars */}
      <div className="card p-5">
        <div className="text-sm font-semibold mb-4" style={{ color: 'var(--text-primary)' }}>Distribución de Calidad de Base de Datos</div>
        <div className="space-y-3">
          {[
            { label: 'Alta confianza', value: aud.alta, pct: aud.pctAlta, color: '#166534' },
            { label: 'Confianza media', value: aud.media, pct: aud.pctMedia, color: '#d97706' },
            { label: 'Baja confianza', value: aud.baja, pct: aud.pctBaja, color: '#dc2626' },
          ].map(bar => (
